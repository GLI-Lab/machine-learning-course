...
This reads `pixi.lock` and installs every package at its exact pinned version. Because no version resolution is needed, this step is fast. The following packages are installed as part of the course environment — you do not need to install them separately.

- `numpy`: array operations and linear algebra
- `matplotlib`: visualization
- `jupyterlab`: interactive notebook interface
- `torch` and `torchvision`: deep learning (used from Unit 4 onward)
