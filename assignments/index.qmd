...
| HW | Title | Due |
|:---|:-----|:-----|
| [HW01](hw01/hw01.html) | Numerical Programming with NumPy | ~~26.03.10~~ 26.03.13 |
...
- `hwXX.py` (or multiple `hwXX_part.py` files): **Submission file(s).** Write your solutions here (e.g., `hw0_split.py`, `hw0_knn.py`).
...
1. **Submit Code**: Upload only the requested `.py` files (e.g., `hw0_split.py` and `hw0_knn.py`) to [Konkuk eCampus](https://ecampus.konkuk.ac.kr/). **Do not submit the `.ipynb` notebooks.**
2. **Do not rename the files.** Submitting a file with a modified name (e.g., `hw0_split (1).py`) may result in zero credit for that file, as the autograder identifies submissions by exact filename.
